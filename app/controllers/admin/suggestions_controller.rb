class Admin::SuggestionsController < Admin::BaseController
  ...
  def available
    if params[:q].blank?
      ...
      @available_suggestions = Product.find(:all,:include=>[:variants,:globalize_translations],:conditions => ['lower(name) LIKE ? OR lower(variants.sku) LIKE ?', "%#{params[:q].downcase}%", "%#{params[:q].downcase}%"])
    end
